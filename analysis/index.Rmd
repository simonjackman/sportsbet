...
lastDate <- strftime(max(d$datetime),"%l%p %e %B")
...
- Sportsbet is offering odds in `r length(unique(dsum$event))` key House seats.  `r nrow(tab_change)` of these seats are priced as changing hands; see section \@ref(seats-changing-hands) below.
...
The following `r nrow(tab_change)` seats have candidates of incumbent parties with IPOWs of less than 50%:
