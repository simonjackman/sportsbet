...
tab_change <- plotData %>% 
  ...
  summarise(incumbent = outcomes_collapsed[which.max(p)],
            ...
            incumbent_fav = !any(prob>incumbent_prob),
            favorite = outcomes_collapsed[which.max(prob)]) %>% 
  ungroup()

tab_change <- left_join(
  tab_change %>% filter(!incumbent_fav),
...
datatable(tab %>% 
            mutate(datetime = str_squish(strftime(datetime,"%l%p %a %d %b"))),
          rownames=FALSE,
...
The following `r nrow(tab_change)` seats do **not** have the candidate of the incumbent party as the market favourite (nor among the favourites, in the rare case of ties).
...
```{r h-fluc-week-plot,fig.height=1.25*ceiling(panels/2),eval=panels>0}
...
```{r h-fluc-month,fig.height=8,fig.width=8,eval=FALSE}
h_fluc_month <- d %>%
  filter(lab %in% theStates) %>%
  group_by(event) %>%
  arrange(desc(datetime)) %>%
