...
- Sportsbet is currently offering odds in `r currentSeats` of 151 House of Representative seats; we use the most recently available odds for the remaining `r 151-currentSeats` seats. 

- `r nrow(tab_change)` seats are priced as changing hands; see section \@ref(seats-changing-hands) below. 
...
winning implied by Sportsbet's odds as at `r lastDate`.   In seats where Sportsbet is not currently offering prices we use the last observed odds in that seat.
...
datatable(tab %>% 
            mutate(datetime = str_squish(strftime(tab$datetime,"%l%p %a %d %b"))),
          rownames=FALSE,
...
The following `r nrow(tab_change)` seats have candidates of incumbent parties with IPOWs of less than 50% (again, with long-shot probabilities removed):
...
h_fluc_24 <- d %>%
  ...
  filter(as.Date(datetime,tz="Australia/Sydney")>(Sys.Date()-2)) %>%
  ungroup() %>%
  group_by(event,datetime) %>%
  mutate(prob_nls = ifelse(prob<.10,0,prob)) %>%
  mutate(prob_nls = prob_nls/sum(prob_nls)) %>%
  ungroup() %>%
  arrange(event,outcomes,datetime)

h_fluc_24 <- h_fluc_24 %>%
  semi_join(h_fluc_24 %>%
              group_by(event,outcomes) %>%
              summarise(flag = max(prob_nls)!=min(prob_nls)) %>%
              ungroup() %>%
              filter(flag) %>%
              distinct(event),
            by="event"
...
Seats with odds changing in the last 24 hours: `r if(nrow(h_fluc_24)==0){"none"}`

```{r eval=nrow(h_fluc_24)>0}
knitr::kable(h_fluc_24 %>% select(event,outcomes,datetime,prob_nls))
```
...
if(nrow(h_fluc_week)>0){
  ...
  g <- ggplot(h_fluc_week,
              aes(x=datetime,y=prob_nls*100,group=outcomes,col=outcomes)) + 
    geom_step() + 
    scale_x_datetime("",minor_breaks = NULL,timezone = "Australia/Sydney",date_labels = "%e %b") + 
    scale_y_continuous("IPOW",minor_breaks = NULL) + 
    scale_color_manual("",values=c("Coalition"="#009de3","Labor"="#ed1b35")) + 
    facet_wrap(~event) + 
...
h_fluc_month <- d %>%
  ...
  group_by(event) %>%
  arrange(desc(datetime)) %>%
  filter(as.Date(datetime,tz="Australia/Sydney")>(Sys.Date()-29)) %>%
  ungroup() %>%
  group_by(event,datetime) %>%
  mutate(prob_nls = ifelse(prob<.10,0,prob)) %>%
  mutate(prob_nls = prob_nls/sum(prob_nls)) %>%
...
if(nrow(h_fluc_month)>0){
  ...
  g <- ggplot(h_fluc_month,
              aes(x=datetime,y=prob_nls*100,group=outcomes,col=outcomes)) + 
    geom_step() + 
    scale_x_datetime("",minor_breaks = NULL,timezone = "Australia/Sydney",date_labels = "%e\n%b") + 
    scale_y_continuous("IPOW",minor_breaks = NULL) + 
    scale_color_manual("",values=c("Coalition"="#009de3","Labor"="#ed1b35")) + 
    facet_wrap(~event) + 
    theme_minimal(base_family = "Avenir") + 
