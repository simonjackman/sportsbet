...
s2 <- dsum %>% 
  group_by(event) %>% 
  mutate(leader = prob==max(prob,na.rm=TRUE)) %>%
  ungroup() %>%
...
tab_change <- plotData %>% 
  ...
  group_by(Division) %>% 
  summarise(incumbent = outcomes_collapsed[which.max(p_tcp)],
            incumbent_tcp = p_tcp[which.max(p_tcp)],
            incumbent_prob = prob[which.max(p_tcp)],
...
### `r nrow(tab)` seats with odds changing in the last 24 hours {.unnumbered}
...
### `r panels` seats with change in the last week {.unnumbered}
...
### `r panels` seats with odds changing hands last 28 days
